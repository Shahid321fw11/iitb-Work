...
/// Fetch user selection from the form submission
if (isset($_POST['user'])) {
    $currentuser = mysqli_real_escape_string($link, $_POST['user']);
} else {
    $currentuser = $_SESSION["ldap"];
}
                            ...
                            <table width="80%" align="center" border="0">
                                <tr>
                                    <td>
                                        ...
                                        <?php if ($all == 1) { ?><option value="all">All Users</option><?php } ?> // this one is commented previously.
                                        <?php
                                        ...
                                        <select name="user" id="user">
                                            <?php
                                            // Prepare SQL to fetch employees for the dropdown
                                            $worker_ids_string = implode(",", $worker_ids);
                                            $sql = "SELECT memberid, fname, lname FROM login WHERE memberid IN ($worker_ids_string)";

                                            $usersql = mysqli_query($link1, $sql);

                                            if (!$usersql) {
                                                echo "<option>Error loading employees: " . mysqli_error($link1) . "</option>";
                                            } else {
                                                while ($userrow = mysqli_fetch_array($usersql)) {
                                                    echo "<option value='" . $userrow["memberid"] . "'>" . $userrow["fname"] . " " . $userrow["lname"] . "</option>";
                                                }
                                            }
                                            ?>
                                        </select>
                                    ...
                                    <td>
                                        <!-- <b>Select Date: <font color="#FF0000">*</font></b> -->
                        ...
                        <table width="100%" cellspacing="0" align="center" border="0" class="manage" id="dailyentry">
                            ...
                            if (isset($_REQUEST["searchuser"])) {
                                ...
                                if ($_REQUEST["user"] == "all") {
                                    ...
                                    $sql .= " and ldap='" . mysqli_real_escape_string($link, $currentuser) . "'";
                                        ...
                                        <tr>
                                            ...
                                            <?php if ($_REQUEST["user"] != "all") { ?>
                                                <td align="center" valign="top">
                                                    <a href="manager_reporting_entry.php?frompage=detail&user=<?php echo $_REQUEST["user"]; ?>&datepicker=<?php echo
